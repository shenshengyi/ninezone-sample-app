...
export class UnifiedSelectionPropertyGridWidgetControl extends WidgetControl {
  constructor(info: ConfigurableCreateInfo, options: any) {
    ...
    if (options && options.iModelConnection)
      this.reactNode = (
        <UnifiedSelectionPropertyGridWidget
...
class UnifiedSelectionPropertyGridWidget extends React.Component<
  UnifiedSelectionPropertyGridWidgetProps,
  State
> {
  ...
  private async buildContextMenu(args: PropertyGridContextMenuArgs) {
    const field = await this.state.dataProvider.getFieldByPropertyRecord(
      args.propertyRecord
    );
      ...
      if (
        ...
        items.push({
          ...
          title: IModelApp.i18n.translate(
            "SampleApp:properties.context-menu.remove-favorite.description"
          ),
          label: IModelApp.i18n.translate(
            "SampleApp:properties.context-menu.remove-favorite.label"
          ),
        });
      } else {
        items.push({
          ...
          title: IModelApp.i18n.translate(
            "SampleApp:properties.context-menu.add-favorite.description"
          ),
          label: IModelApp.i18n.translate(
            "SampleApp:properties.context-menu.add-favorite.label"
          ),
        });
      }
    }
  ...
  private _favoriteActionButtonRenderer = (
...
function UnifiedSelectionPropertyGrid(
    ...
    return (
      <FillCentered>
        {IModelApp.i18n.translate(
          "SampleApp:property-grid.too-many-elements-selected"
        )}
      </FillCentered>
    );
  }
...
ConfigurableUiManager.registerControl(
  "UnifiedSelectionPropertyGridDemoWidget",
  UnifiedSelectionPropertyGridWidgetControl
);
...
class FavoriteActionButton extends React.Component<FavoriteActionButtonProps> {
  ...
  private async toggleFavoriteProperty() {
    ...
    if (this._isMounted) this.setState({ isFavorite: this.isFavorite() });
  }
